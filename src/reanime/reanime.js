      ...
      const dayIndex =
        videos.filter(
          v =>
            v.season == season &&
            v.released &&
            v.released.split('T')[0] === targetDate &&
            parseInt(v.episode) < parseInt(episode)
        ).length + 1;
...
export async function resolveByDate(
  releaseDateStr,
  showTitle,
  originalEpisode,
  episodeTitle,
  ...
  try {
    ...
    for (const anime of candidates) {
      ...
      const diffDays = Math.ceil(
        Math.abs(targetDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)
      );
      ...
      if (isMatch) {
        ...
        if (episodes.length > 1 && episodeTitle) {
          ...
          for (let j = 0; j < episodes.length; j++) {
            ...
            if (
              cleanAl &&
              (cleanAl.indexOf(cleanTarget) !== -1 || cleanTarget.indexOf(cleanAl) !== -1)
            ) {
              episodeNum = j + 1;
              break;
            }
...
function scoreCandidate(title, query, year, targetAnilistId, candidateAnilistId) {
  if (
    targetAnilistId &&
    candidateAnilistId &&
    String(targetAnilistId) === String(candidateAnilistId)
  ) {
    return 1000;
  }
          ...
          list.forEach(item => {
            ...
            if (cleanSlug) {
              ...
              candidates.push({
                slug: cleanSlug,
                title:
                  item.title?.english || item.title?.romaji || item.title || item.name || cleanSlug,
                anilistId: alId,

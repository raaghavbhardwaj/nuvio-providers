...
async function decryptVidStack(encoded, url) {
  ...
  for (const ivStr of ivs) {
    try {
      ...
      const decrypted = CryptoJS.AES.decrypt({ ciphertext: CryptoJS.enc.Hex.parse(encoded) }, key, {
        iv: iv,
  ...
  try {
    ...
    if (form.length > 0) {
      ...
      const postUrl = action.startsWith('http')
        ? action
        : `${urlObj.protocol}//${urlObj.hostname}${action}`;
...
export async function doodStreamExtractor(url) {
  try {
    ...
    const passRes = await fetch(`https://dood.re/pass_md5/${md5}`, {
...
export async function loadExtractor(url) {
  try {
    ...
    if (
      domain.includes('fsdcmo') ||
      domain.includes('hglink') ||
      domain.includes('dingtezuni') ||
      domain.includes('vidhide') ||
      domain.includes('earnvids') ||
      domain.includes('filelions') ||
      domain.includes('smoothpre') ||
      domain.includes('dhtpre') ||
      domain.includes('peytonepre') ||
      domain.includes('ryderjet') ||
      domain.includes('abstream')
    ) {
      return await vidHideExtractor(url);
    }
    if (
      domain.includes('vidstack') ||
      domain.includes('hubstream') ||
      domain.includes('bigwarp') ||
      domain.includes('mxdrop') ||
      domain.includes('wasuytm') ||
      domain.includes('upn.one') ||
      domain.includes('server1.uns') ||
      domain.includes('kumi.uns')
    ) {

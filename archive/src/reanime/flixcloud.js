...
function normalizeFlixEmbedUrl(url, referer) {
  let finalUrl = url.startsWith('http')
    ? url
    : `https://flixcloud.cc${url.startsWith('/') ? '' : '/'}${url}`;
...
function _wasmBlockEnds(body, start) {
  ...
  let pc = start;
  const readUleb = () => {
    while (pc < body.length && (body[pc++] & 0x80) !== 0) {}
  };
  while (pc < body.length) {
    const opPc = pc,
      op = body[pc++];
    switch (op) {
      ...
      case 0x03:
        ...
        break;
      case 0x0b:
        if (stack.length > 0) ends.set(stack.pop(), opPc);
        break;
      case 0x0c:
      case 0x0d:
      case 0x20:
...
function uint8ArrayToBase64(arr) {
  ...
  for (let i = 0; i < bin.length; i += 3) {
    let a = bin.charCodeAt(i),
      b = bin.charCodeAt(i + 1),
      c = bin.charCodeAt(i + 2);
    output += chars[a >> 2];

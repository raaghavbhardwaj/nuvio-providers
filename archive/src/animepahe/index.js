...
async function getStreams(tmdbId, mediaType, season, episode) {
  try {
    ...
    $('div#pickDownload a').each((i, el) => {
      ...
      if (
        paheUrl &&
        (paheUrl.includes('pahe.win') ||
          paheUrl.includes('pahe.me') ||
          paheUrl.includes('pahe.li') ||
          paheUrl.includes('kwik'))
      ) {
        promises.push(
          extractPahe(paheUrl)
            .then(res => {
...
async function onSettings() {
  return [
    { type: 'header', label: 'Domain Selection' },
    {
      ...
      description:
        'AnimePahe frequently rotates domains. Choose the one currently working for you.',
      options: [
